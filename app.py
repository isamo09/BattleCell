...
def login():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            cursor.execute(
                'SELECT id, username, password_hash, master_password_hash FROM users WHERE LOWER(username) = LOWER(?)',
                (username,))
            user_data = cursor.fetchone()
            if user_data:
...
def logout():
    ...
    # Clear any master password session data
    if user_id:
        session.pop(f'master_password_verified_{user_id}', None)
        session.pop(f'master_key_{user_id}', None)
...
def passwords():
    ...
    try:
        cursor.execute(
            'SELECT id, title_encrypted, username_encrypted, url_encrypted, notes_encrypted, created_at FROM passwords WHERE user_id = ?',
            (current_user.id,))
        passwords_data = cursor.fetchall()
    except sqlite3.OperationalError:
        # Если новая структура не существует, используем старую
        try:
            cursor.execute('SELECT id, title, username, url, notes, created_at FROM passwords WHERE user_id = ?',
                           (current_user.id,))
            passwords_data = cursor.fetchall()
            conn.close()
...
def add_password():
    ...
    cursor = conn.cursor()
    cursor.execute(
        'INSERT INTO passwords (user_id, title_encrypted, username_encrypted, password_encrypted, url_encrypted, notes_encrypted) VALUES (?, ?, ?, ?, ?, ?)',
...
def get_password(password_id):
    ...
    try:
        cursor.execute(
            'SELECT title_encrypted, username_encrypted, password_encrypted, url_encrypted, notes_encrypted FROM passwords WHERE id = ? AND user_id = ?',
            (password_id, current_user.id))
        password_result = cursor.fetchone()
    except sqlite3.OperationalError:
        # Если новая структура не существует, используем старую
        cursor.execute('SELECT title, username, password, url, notes FROM passwords WHERE id = ? AND user_id = ?',
                       (password_id, current_user.id))
        password_result = cursor.fetchone()
...
def edit_password(password_id):
    ...
    cursor.execute(
        'UPDATE passwords SET title_encrypted = ?, username_encrypted = ?, password_encrypted = ?, url_encrypted = ?, notes_encrypted = ? WHERE id = ? AND user_id = ?',
        (encrypted_title, encrypted_username, encrypted_password, encrypted_url, encrypted_notes, password_id,
         current_user.id))
...
def files():
    ...
    # Проверяем структуру таблицы
    try:
        cursor.execute('SELECT id, original_filename_encrypted, file_size, created_at FROM files WHERE user_id = ?',
                       (current_user.id,))
        files_data = cursor.fetchall()
    except sqlite3.OperationalError:
        # Если новая структура не существует, используем старую
        try:
            cursor.execute('SELECT id, original_filename, file_size, created_at FROM files WHERE user_id = ?',
                           (current_user.id,))
            files_data = cursor.fetchall()
            conn.close()
...
def upload_file():
    try:
        ...
        if not user_result or not user_result[0]:
            return jsonify(
                {'error': 'Мастер-пароль не установлен. Установите мастер-пароль в настройках аккаунта.'}), 400
        ...
        if file_size > 500 * 1024 * 1024:  # 500MB
            return jsonify(
                {'error': f'Файл слишком большой. Размер: {format_size(file_size)}, максимальный размер: 500MB'}), 400
        ...
        if total_storage_after_upload > max_storage:
            return jsonify({
                               'error': f'Недостаточно места. Доступно: {format_size(max_storage - current_storage)}, требуется: {format_size(file_size)}'}), 400
        ...
        try:
            cursor.execute(
                'INSERT INTO files (user_id, filename_encrypted, original_filename_encrypted, file_size, encrypted_data) VALUES (?, ?, ?, ?, ?)',
...
def download_file(file_id):
    ...
    try:
        cursor.execute('SELECT original_filename_encrypted, encrypted_data FROM files WHERE id = ? AND user_id = ?',
                       (file_id, current_user.id))
        file_result = cursor.fetchone()
    except sqlite3.OperationalError:
        # Если новая структура не существует, используем старую
        cursor.execute('SELECT original_filename, encrypted_data FROM files WHERE id = ? AND user_id = ?',
                       (file_id, current_user.id))
        file_result = cursor.fetchone()
...
def change_username():
    ...
    try:
        ...
        try:
            ...
            cursor.execute('SELECT id FROM users WHERE username_encrypted = ? AND id != ?',
                           (encrypted_new_username, current_user.id))
            if cursor.fetchone():
                conn.close()
                flash('Это имя пользователя уже занято')
                return redirect(url_for('settings'))
