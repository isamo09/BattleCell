  ...
  document.addEventListener("keydown", (e) => {
    if (e.ctrlKey || e.metaKey) {
      switch (e.key) {
        case "k":
          e.preventDefault()
          if (window.location.pathname === "/passwords" && typeof window.showAddPasswordModal === "function") {
            window.showAddPasswordModal()
          } else if (window.location.pathname === "/files" && typeof window.showUploadFileModal === "function") {
            window.showUploadFileModal()
          }
          break
        case "s":
...
const searchPasswords = debounce((query) => {
  ...
  passwordCards.forEach((card) => {
    ...
    notesElements.forEach((element, index) => {
      if (index > 0) {
        // Пропускаем первый элемент (username)
...
const searchFiles = debounce((query) => {
